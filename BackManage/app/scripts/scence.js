...
"use strict";
var back, city_init, clearmap, commit, get_scene, get_scenes, is_city, is_prov_city, is_province, mark, markbiao, move, moveback, prov_city, root, save, search, substringMatcher,
...
is_province = function(name) {
  if (__indexOf.call(prov_city.p, name) >= 0) {
    return true;
  } else {
    return false;
  }
...
is_city = function(name) {
  if (__indexOf.call(prov_city.c, name) >= 0) {
    return true;
  } else {
    return false;
  }
...
is_prov_city = function(name) {
  if (__indexOf.call(prov_city.p, name) >= 0 || prov_city.c) {
    return true;
  } else {
    return false;
  }
...
get_scenes = function(name) {
  if (!is_prov_city(name)) {
    return false;
  }
  scene_map.setZoom(is_province(name) ? 7 : 12);
...
markbiao = function(e) {
  ...
  return console.log(marker1.Size);
};

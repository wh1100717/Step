        ...
        while times < 10:
            try:
                response = requests.get(
                    url, headers={'User-Agent': AgentPoolUtil.getRandomUserAgent()})
                response.encoding = Config['encoding']
...
if __name__ == '__main__':
    ...
    # 设置需要抓取的城市
    parser.add_argument('-c', type=str, dest="city_cn", default="北京",
                        help="config the city, input chinese city name, 北京 by default")

    # 设置存储格式
    parser.add_argument('-s', type=str, dest="save_type", default="mongo",
                        help="config the save mode: mongo | file, mongo by default")
